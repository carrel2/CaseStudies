(function ($) {
    ...
    var init = function (index, input, options) {
        ...
        $canvas.css({"background-image": 'url(' + image.src + ')', "background-size": "contain"});
        ...
        mousedown = function (e) {
            ...
            var rgb = document.getElementById("canvas-hidden").getContext("2d").getImageData(x, y, 1, 1);
            var id = rgb.data[0];

            if(id) {
              var h = $.data(document.body, "hotspots")[id - 1];
              var name = $("div.hotspot#hotspot-" + id).text();

              points = [].concat(h.coords);

              $("input#name").val(name.slice(0, name.indexOf(" ")));
        ...
        $(document).find($canvas).on('mousemove', draw);

    };

    $(document).ready(function () {
        $('.canvas-area[data-image]').canvasAreaDraw();
    });
